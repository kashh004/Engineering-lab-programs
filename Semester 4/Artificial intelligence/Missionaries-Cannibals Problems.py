...
defis_valid(self):
ifself.missionaries< 0 or self.cannibals< 0 or self.missionaries> 3 or self.cannibals> 3:
return False
ifself.missionaries> 0 and self.missionaries<self.cannibals:
return False
if 3 - self.missionaries> 0 and 3 - self.missionaries< 3 - self.cannibals:
return False
return True
defis_goal(self):
returnself.missionaries == 0 and self.cannibals == 0 and self.boat == 0
def successors(self):
successors = []
ifself.boat == 1:
...
return path[::-1]
defbest_first_search(initial_state):
...
returnstate.missionaries + state.cannibals
...
# Print the solution
print("Solution:")
fori, step in enumerate(solution):
print("Step {}: {} Missionaries, {} Cannibals, Boat on side {}".format(i+1, step[0], step[1], "Starting" if
step[2] == 1 else "Ending"))
